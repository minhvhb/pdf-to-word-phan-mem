...
def extract_text_from_file(uploaded_file, client):
    ...
    if file_ext == "docx":
        ...
        # BƯỚC 1: Thử đọc chữ trực tiếp bằng pypdf (Miễn phí, cực nhanh)
        try:
            reader = PdfReader(uploaded_file)
            extracted = "\n".join([p.extract_text() or "" for p in reader.pages]).strip()
            if len(extracted) > 50: # Nếu tìm thấy nội dung chữ (Digital PDF)
                uploaded_file.seek(0) # Reset con trỏ file sau khi đọc
                return extracted
        except Exception:
            pass
            
        # BƯỚC 2: Nếu là file scan/ảnh (không có chữ thuần), mới gọi Gemini
        uploaded_file.seek(0) # Đảm bảo file được đọc từ đầu
